...
def safe_str(v: Any, max_token_len: int = 60) -> str:
    def _sanitize_for_latin1(text: str) -> str:
        """Replace common Unicode characters with ASCII/Latin-1-safe equivalents.

        This prevents fpdf (which uses latin-1 for core fonts) from raising
        Unicode encoding errors when rendering summaries or other text that may
        ...
        # Finally, ensure the string is encodable to latin-1; if not, replace
        try:
            text.encode("latin-1")
            return text
        except Exception:
